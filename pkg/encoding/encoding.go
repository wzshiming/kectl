...
func Convert(codecs serializer.CodecFactory, inMediaType, outMediaType string, in []byte) (*runtime.TypeMeta, []byte, error) {
	...
	inCodec, err := newCodec(codecs, gv, inMediaType)
	if err != nil {
		return nil, nil, err
	}

	outCodec, err := newCodec(codecs, gv, outMediaType)
	if err != nil {
		return nil, nil, err
	}
...
func newCodec(codecs serializer.CodecFactory, gv schema.GroupVersion, mediaType string) (runtime.Codec, error) {
...
func MediaTypeFromGVR(codecs serializer.CodecFactory, gvr schema.GroupVersionResource) (mediaType string, err error) {
	...
	if !strings.Contains(gvr.Group, ".") || strings.HasSuffix(gvr.Group, ".k8s.io") {
		if _, err := newCodec(codecs, gvr.GroupVersion(), StorageBinaryMediaType); err == nil {
			return StorageBinaryMediaType, nil
		}
	}
